...
describe('Kadys', () => {
  ...
  beforeEach(async () => {
    ...
    kadysJettonWallet = blockchain.openContract(
      await JettonDefaultWallet.fromInit(minter.address, kadys.address),
    );
    await kadys.send(
      deployer.getSender(),
      {
  ...
  describe('Stake', () => {
    beforeEach(async () => {
      const senderWalletAddress = await minter.getGetWalletAddress(
        deployer.address,
      );
      const senderWallet = blockchain.openContract(
        JettonDefaultWallet.fromAddress(senderWalletAddress),
      );
  ...
  describe('Unstake', () => {
    ...
    it('should emit UnstakeEvent', async () => {
      ...
      stakeRes.externals.forEach((ext) => {
        expect(loadUnstakeEvent(ext.body.asSlice()).$$type).toEqual(
          'UnstakeEvent',
        );
      });
    });
  ...
  describe('Earned', () => {
    describe('Only stake', () => {
      ...
      it('should return correct earned after deposit with 1 TON and 1 week passed', async () => {
        ...
        const expectedEarned =
          (Number(earnedInOneDay) * 7) / Number(scaleFactor);
        expect(earnedFromNano).toBeCloseTo(expectedEarned);
      });
      it('should return correct earned after deposit with 1 TON and 1 year passed', async () => {
        ...
        const expectedEarned =
          (Number(earnedInOneDay) * 365) / Number(scaleFactor);
        expect(earnedFromNano).toBeCloseTo(expectedEarned);
      });
      ...
      it('should return correct earned after 2 deposit with 1 TON with 1 day gap', async () => {
        ...
        expect(earnedFromNano).toBeCloseTo(
          expectedEarned / Number(scaleFactor),
        );
      });
      it('should return correct earned after 2 deposit with 1 TON with 1 week gap', async () => {
        ...
        const earnedFromNano = fromEarnedToNumber(earned);
        const expectedEarned =
          Number(earnedInOneDay) * 7 + Number(earnedInOneDay) * 2 * 7;
        expect(earnedFromNano).toBeCloseTo(
          expectedEarned / Number(scaleFactor),
        );
      });
      it('should return correct earned after 2 deposit with 1 TON with 1 year gap', async () => {
        ...
        const earnedFromNano = fromEarnedToNumber(earned);
        const expectedEarned =
          Number(earnedInOneDay) * 365 + Number(earnedInOneDay) * 2 * 365;
        expect(earnedFromNano).toBeCloseTo(
          expectedEarned / Number(scaleFactor),
        );
      });
    ...
    it('should return correct earned after deposit and unstake', async () => {
      ...
      const earnedFromNano = fromEarnedToNumber(earned);
      const expectedEarned =
        numerizedEarnedInOneDay * 7 * 100 + numerizedEarnedInOneDay * 7 * 50;
  ...
  describe('WithdrawEarned', () => {
    ...
    it('should withdraw and update rewards', async () => {
      ...
      const earnedFromNano = fromEarnedToNumber(earned);
      const expectedEarned =
        (Number(earnedInOneDay) * 100 * 365) / Number(scaleFactor);
      expect(earnedFromNano).toBeCloseTo(expectedEarned);
      const response = await kadys.send(
        deployer.getSender(),
        {
      ...
      const afterWithdrawJettonData =
        await deployerJettonWallet.getGetWalletData();
      console.log(
        '=>(Kadys.spec.ts:736) afterWithdrawJettonData',
        afterWithdrawJettonData,
      );
      expect(afterWithdrawJettonData.balance).toBe(
        initialJettonData.balance + toNano(1),
      );
      expect(response.transactions).toHaveTransaction({
        from: deployer.address,
        to: kadys.address,
      ...
      const earnedAfterWithdraw = await kadys.getEarnedOfAddress(
        deployer.address,
      );
      const earnedAfterWithdrawFromNano =
        fromEarnedToNumber(earnedAfterWithdraw);
