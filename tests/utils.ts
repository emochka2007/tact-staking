...
export const sendJettonTest = async ({
  jettonWallet,
  sender,
  amount,
  destination,
...
export const sendJetton = async ({
  provider,
  destination,
  amount,
}: SendJettonProps) => {
  const ownerJetton = Address.parse(process.env.OWNER_JETTON!!);
  const jettonWallet = provider.open(
    JettonDefaultWallet.fromAddress(ownerJetton),
  );
  return await jettonWallet.send(
    provider.sender(),
    {
